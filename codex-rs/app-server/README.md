...
- `thread/list` — page through stored rollouts; supports cursor-based pagination and optional `modelProviders`, `sourceKinds`, `archived`, `cwd`, and `searchTerm` filters. Each returned `thread` includes `status` (`ThreadStatus`), defaulting to `notLoaded` when the thread is not currently loaded. Search results may also include `searchPreview` context for UI rendering.
...
- `searchTerm` — restrict results to threads whose extracted title or persisted user/assistant conversation text contains this substring (case-sensitive). Matching results may include `thread.searchPreview` with either a compact opening conversation preview or a matched content snippet.
