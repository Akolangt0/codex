...
- `thread/search` — experimental search endpoint for persisted rollout contents. It requires `searchTerm`, supports source/archive filters plus cursor pagination, and returns result rows with `thread` metadata and a matching `searchPreview` snippet for UI rendering.
